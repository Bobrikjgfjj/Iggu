...
class Tapper:
    ...
    async def login(self, http_client: aiohttp.ClientSession, tg_web_data: str) -> str:
        ...
        try:
            ...
            logger.error(f"{self.session_name} | Неизвестная ошибка при получении токена: {error} | "
    ...
    async def get_me_telegram(self, http_client: aiohttp.ClientSession) -> dict[str]:
        ...
        try:
            ...
            logger.error(f"{self.session_name} | Неизвестная ошибка при получении данных тг: {error} | "
                         f"Response text: {escape_html(response_text)[:128]}...")
            await asyncio.sleep(delay=3)
    ...
    async def get_config(self, http_client: aiohttp.ClientSession) -> dict[str]:
        ...
        try:
            ...
            logger.error(f"{self.session_name} | Неизвестная ошибка при получении конфигурации: {error} | "
                         f"Response text: {escape_html(response_text)[:128]}...")
            await asyncio.sleep(delay=3)
    ...
    async def select_exchange(self, http_client: aiohttp.ClientSession, exchange_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"{self.session_name} | Ниезвестная ошибка при выборе бижы: {error} | "
                         f"Response text: {escape_html(response_text)[:128]}...")
            await asyncio.sleep(delay=3)
    ...
    async def get_daily(self, http_client: aiohttp.ClientSession):
        ...
        try:
            ...
            logger.error(f"{self.session_name} | Неизвестная ошибка при ежедневном получения вознограждения: {error} | "
                         f"Response text: {escape_html(response_text)[:128]}...")
            await asyncio.sleep(delay=3)
    ...
    async def apply_boost(self, http_client: aiohttp.ClientSession, boost_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"{self.session_name} | Неизвестная ошибка при прокачки {boost_id} Boost: {error} | "
                         f"Response text: {escape_html(response_text)[:128]}...")
            await asyncio.sleep(delay=3)
            ...
            while True:
                try:
                    if time() - access_token_created_time >= 3600:
                        ...
                        logger.info(f"{self.session_name} | Последний пассивный заработок: <g>+{last_passive_earn}</g> | "
                        ...
                        if is_completed is False:
                            ...
                            if status is True:
                                logger.success(f"{self.session_name} | Успешно получино ежедневное вознаграждение | "
                                               f"Days: <m>{days}</m> | Получено монет: {rewards[days - 1]['rewardCoins']}")
                        ...
                        if not exchange_id:
                            ...
                            if status is True:
                                logger.success(f"{self.session_name} | Успешно выбранный биржы <y>Bybit</y>")
                    ...
                    logger.success(f"{self.session_name} | Успешно накоплено | "
                                   f"Баланс: <c>{balance}</c> (<g>+{calc_taps}</g>) | Итог: <e>{total}</e>")
                    ...
                    if active_turbo is False:
                        if (settings.APPLY_DAILY_ENERGY is True
                            ...
                            logger.info(f"{self.session_name} | Подождите 5 секунд, прежде чем приступить к бусту энергии")
                            await asyncio.sleep(delay=5)

                            status = await self.apply_boost(http_client=http_client, boost_id="BoostFullAvailableTaps")
                            if status is True:
                                logger.success(f"{self.session_name} | Успешно использован буст энергии")
                        ...
                        if settings.AUTO_UPGRADE is True:
                            for _ in range(settings.UPGRADES_COUNT):
                                ...
                                logger.info(f"{self.session_name} | Подождите 5s перед прокачкой <e>{upgrade_id}</e>")
                                await asyncio.sleep(delay=5)
                                ...
                                if status is True:
                                    ...
                                    logger.success(
                                        f"{self.session_name} | "
                                        f"Успешная прокачка <e>{upgrade_id}</e> потрачено <r>{price:,}</r> прокачена до <m>{level}</m> lvl | "
                                        f"Зарабатак каждый час: <y>{earn_on_hour:,}</y> (<g>+{profit}</g>) | "
                                        f"Баланс: <e>{balance:,}</e>")
                        ...
                        if available_energy < settings.MIN_AVAILABLE_ENERGY:
                            ...
                            logger.info(f"{self.session_name} | Достигнута минимальная энергия: {available_energy}")
                            logger.info(f"{self.session_name} | Подождите {random_sleep}s")
                    ...
                    logger.info(f"Подождите {sleep_between_clicks}s")
...
async def run_tapper(tg_client: Client, proxy: str | None):
    try:
        ...
        logger.error(f"{tg_client.name} | Испорченная сессия")
